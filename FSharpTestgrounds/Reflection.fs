...
type ReflectionTargetAttribute() =
    inherit Attribute()
...
let getTargetMethods =
    getMethodsWithAttribute<ReflectionTargetAttribute>

let chooseMethod (methods: MethodInfo []) =
    let methodsIndexed =
        methods |> Array.mapi (fun i m -> string i, m)

    let map = methodsIndexed |> Map.ofArray
